...
img_size = [416, 416]
yolo_model = yolov3tiny(class_num, anchors)
yolo_model.set_img_size(np.asarray(img_size))

sess = tf.Session(config=config)
with gfile.FastGFile("./pb_model/frozen_model_v3tiny.pb",'rb') as f:
...
feature_map_1 = sess.graph.get_tensor_by_name('yolov3tiny/head/feature_map_1:0')
feature_map_2 = sess.graph.get_tensor_by_name('yolov3tiny/head/feature_map_2:0')
pred_boxes, pred_confs, pred_probs = yolo_model.predict([feature_map_1, feature_map_2])
