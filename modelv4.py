...
from utils.layer_utils import CSPdarknet53_body, SSP, MISH, PANet
...
class yolov4():
    ...
    def forward(self, inputs, is_training=False, reuse=False):
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm], reuse=reuse):
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('CSPdarknet53_body'):
                    route_1, route_2, route_3 = CSPdarknet53_body(inputs)
            with slim.arg_scope([slim.conv2d],
                                normalizer_fn=slim.batch_norm,
                                normalizer_params=batch_norm_params,
                                biases_initializer=None,
                                ...
                                weights_regularizer=slim.l2_regularizer(self.weight_decay)):
                with tf.variable_scope('SSP'):
                    route_3 = SSP(route_3)
                with tf.variable_scope('PANet'):
                    route_up, route_middle, route_down = PANet(route_1, route_2, route_3, self.use_static_shape,
                                                               self.class_num)
                    feature_map_3 = tf.identity(route_up, name="feature_map_3")
                    feature_map_2 = tf.identity(route_middle, name="feature_map_2")
                    feature_map_1 = tf.identity(route_down, name="feature_map_1")
    ...
    def loss_layer(self, feature_map_i, y_true, anchors):
        ...
        # TODO : solve gradient explose of ciou loss
        ciou_loss = xy_loss + wh_loss
        # ciou_loss = self.box_ciou(pred_xy,pred_tw_th,true_xy,true_tw_th)
        # ciou_loss = tf.where(tf.math.greater(object_mask, 0.5), ciou_loss, tf.zeros_like(ciou_loss))
        # ciou_loss = tf.square(ciou_loss * object_mask) * box_loss_scale * mix_w
        ...
        return ciou_loss, conf_loss, class_loss

    def box_ciou(self, pred_box_xy, pred_box_wh, true_box_xy, true_box_wh):
        ...
        union_mins = tf.minimum(pred_box_xy - pred_box_wh / 2.,
                                true_box_xy - true_box_wh / 2.)
        union_maxs = tf.maximum(pred_box_xy + pred_box_wh / 2.,
                                true_box_xy + true_box_wh / 2.)
        union_wh = tf.maximum(union_maxs - union_mins, 0.)
        ...
        # 并集区域对角线
        C = tf.reduce_sum(tf.square(union_wh), -1, keepdims=True)
        # 中心点对角线
        D = tf.reduce_sum(tf.square(true_box_xy - pred_box_xy), -1, keepdims=True)

        pi = 3.14159265358979323846

        v = 4 / (pi * pi) * tf.square(
            tf.subtract(
                tf.math.atan(true_box_wh[..., 0:1] / true_box_wh[..., 1:2]),
                tf.math.atan(pred_box_wh[..., 0:1] / pred_box_wh[..., 1:2])
            )
        )
    ...
    def compute_loss(self, y_pred, y_true):
        ...
        '''
        loss_ciou, loss_conf, loss_class = 0., 0., 0.
        ...
        for i in range(len(y_pred)):
            ...
            loss_conf += result[1]
            loss_class += result[2]
        total_loss = loss_ciou + loss_conf + loss_class
        return [total_loss, loss_ciou, loss_conf, loss_class]
