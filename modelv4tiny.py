...
from utils.layer_utils import conv2d, yolo_Res_block, upsample_layer
...
class yolov4tiny(object):
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm], reuse=reuse):
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope("body"):
                    net = conv2d(inputs, 32, strides=2)
                    net = conv2d(net, 64, strides=2)
                    net, _ = yolo_Res_block(net, 64)
                    net = slim.max_pool2d(net)
                    net, _ = yolo_Res_block(net, 128)
                    net = slim.max_pool2d(net)
                    net, route1 = yolo_Res_block(net, 256)
                with tf.variable_scope("head"):
                    ...
                    route2 = upsample_layer(route2,
                                            route1.get_shape().as_list() if self.use_static_shape else tf.shape(
                                                route1))
                    net = tf.concat([route2, route1], axis=-1)
                    net = conv2d(net, 256)
    ...
    def compute_loss(self, y_pred, y_true):
        '''
        param:
            y_pred: returned feature_map list by `forward` function: [feature_map_1, feature_map_2]
