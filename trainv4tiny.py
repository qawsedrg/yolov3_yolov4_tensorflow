...
import argsv4tiny as args
from modelv4tiny import yolov4tiny
from utils.data_utils import get_batch_data_tiny
...
image_ids, image, y_true13,y_true26 = iterator.get_next()
y_true=[y_true13,y_true26]
...
# Model definition
##################
yolo_model = yolov4tiny(args.class_num, args.anchors, args.use_label_smooth, args.use_focal_loss, args.batch_norm_decay,
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(ep, args.total_epoches):
        ...
        for i in trange(args.train_batch_num):
            ...
            if __global_step % args.train_evaluation_step == 0 and __global_step > 0:
                ...
                recall, precision = evaluate_on_gpu(sess, gpu_nms_op, pred_boxes_flag, pred_scores_flag, __y_pred,
                                                    __y_true, args.class_num, args.nms_threshold,tiny=True)
        ...
        if epoch % args.save_epoch == 0 and epoch > 0:
            saver_to_save.save(sess, args.save_dir + 'model-epoch_{}_step_{}'.format(epoch, int(__global_step)))
        ...
        if epoch % args.val_evaluation_epoch == 0 and epoch >= args.warm_up_epoch:
            ...
            if mAP > best_mAP:
                best_mAP = mAP
                saver_best.save(sess, args.save_dir + 'best_model_mAP_{:.4f}_Epoch_{}_step_{}'.format(
                    best_mAP, epoch, int(__global_step)))
