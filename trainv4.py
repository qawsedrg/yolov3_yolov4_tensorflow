...
# Model definition
##################
yolo_model = yolov4(args.class_num, args.anchors, args.use_label_smooth, args.use_focal_loss, args.batch_norm_decay,
...
tf.summary.scalar('train_batch_statistics/loss_ciou', loss[1])
...
if ckpt and ckpt.model_checkpoint_path:
    step = ckpt.model_checkpoint_path.split('/')[-1].split('_')[-1]
    try:
        step = int(step)
    except:
        step = 0
global_step = tf.Variable(float(step), trainable=False, collections=[tf.GraphKeys.LOCAL_VARIABLES])
if args.use_warm_up:
...
with tf.Session(config=config) as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
        ...
        try:
            step = ckpt.model_checkpoint_path.split('/')[-1].split('_')[-1]
            step = eval(step)
            ep = ckpt.model_checkpoint_path.split('/')[-1].split('_')[-3]
            ep = eval(ep)
        except:
            step = 0
            ep = 0
        try:
            best_mAP = ckpt.model_checkpoint_path.split('/')[-1].split('_')[-5]
            best_mAP = eval(best_mAP)
        except:
            best_mAP = -np.Inf
        print("message: load ckpt model, global_step={:}, mAP={:4f}".format(str(step), float(best_mAP)))
    else:
    ...
    for epoch in range(args.total_epoches):
        ...
        loss_total, loss_ciou, loss_conf, loss_class = AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
        ...
        for i in trange(args.train_batch_num):
            ...
            loss_ciou.update(__loss[1], len(__y_pred[0]))
            ...
            if __global_step % args.train_evaluation_step == 0 and __global_step > 0:
                ...
                info = "Epoch: {}, global_step: {} | loss: total: {:.2f}, ciou: {:.2f}, conf: {:.2f}, class: {:.2f} | ".format(
                    epoch, int(__global_step), loss_total.average, loss_ciou.average, loss_conf.average,
                    loss_class.average)
        ...
        if epoch % args.save_epoch == 0 and epoch > 0:
            saver_to_save.save(sess, args.save_dir + 'model-epoch_{}_step_{}_loss_{:.4f}_lr_{:.5g}'.format(epoch, int(
                __global_step), loss_total.average, __lr))

        # switch to validation dataset for evaluation
        if epoch % args.val_evaluation_epoch == 0 and epoch >= args.warm_up_epoch:
            sess.run(val_init_op)

            val_loss_total, val_loss_ciou, val_loss_conf, val_loss_class = \
            ...
            info += 'EVAL: loss: total: {:.2f}, ciou: {:.2f}, conf: {:.2f}, class: {:.2f}\n'.format(
                val_loss_total.average, val_loss_ciou.average, val_loss_conf.average, val_loss_class.average)
            print(info)
            logging.info(info)
            ...
            writer.add_summary(make_summary('validation_statistics/total_loss', val_loss_total.average),
                               global_step=epoch)
            writer.add_summary(make_summary('validation_statistics/loss_ciou', val_loss_ciou.average),
                               global_step=epoch)
